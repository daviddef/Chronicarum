"""Merge the Italian heritage import into the bundled catalogue.

Same machinery as merge_heritage_uk.py — the dedup rules and columns are identical; only
the source file and the country label differ. Written separately rather than parameterised
because each import has enough of its own quirks that a shared function grows more
conditionals than it saves.
...
def location_for(r):
    """"Rome, Italy" beats a bare country line."""
    admin = (r.get("admin") or "").strip()
    return f"{admin}, Italy" if admin else "Italy"
...
heritage = json.load(open(f"{SP}/heritage_it.json"))
added = enriched = 0
skipped = Counter()
...
for r in heritage:
    ...
    if sid in by_id:
        ...
        # better location line rather than leaving a bare "Italy".
        existing = by_id[sid]
        loc = location_for(r)
        if loc != "Italy" and existing.get("country") in (None, "", "Italy"):
...
print(f"added {added} Italian heritage places, enriched {enriched} existing with a locality")
